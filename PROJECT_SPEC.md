...
- Keep the service rows in their original compact list style even though they now act as disclosure controls for inline booking panels.
...
- **Copy style:** Use conventional title case for headings, interface labels, captions, buttons, and service names. Use `and` instead of an ampersand in those labels. Keep paragraph copy in sentence case and preserve the exact approved display name, `Marie’s Minks n Beauty Bar`.
- **Mobile experience:** Phone layouts use a compact header with a persistent Book Now action, a keyboard-accessible Menu, touch-sized controls, reduced vertical spacing, single-column content stacks, and full-width primary actions. Desktop/tablet compositions remain intact above the phone breakpoint.
